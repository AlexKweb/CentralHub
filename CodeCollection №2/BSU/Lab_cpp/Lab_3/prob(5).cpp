...
// Пример cmp для C-строк (массив указателей на char*)
int cmp_cstr(const void* p1, const void* p2) {
    // здесь p1 и p2 — указатели на элементы массива; если массив — char* arr[],
    // то каждый элемент — (char*), поэтому сначала приводим к (char* const*)
...
int main() {
    ...
    // Пример 2: сортировка массива C-строк (массив указателей)
    ...
    // ВАЖНО: здесь элемент массива имеет размер sizeof(char*), поэтому len = sizeof(char*)
